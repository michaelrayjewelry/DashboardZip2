...
// MAIN PRODUCTS PAGE
// ═══════════════════════════════════════
export default function ZipJewelerProducts() {
    ...
    <div style={{ fontFamily: SANS, background: C.bg, minHeight: "100vh", color: C.dark, display: "flex" }}>
      ...
      <aside style={{
        ...
        <div style={{ padding: "36px 28px", flex: 1 }}>
          <div style={{ fontFamily: SANS, fontSize: 9.5, fontWeight: 600, letterSpacing: 2.5, textTransform: "uppercase", color: "rgba(168,168,156,0.45)", marginBottom: 14 }}>Collections</div>
          {COLLECTIONS.map((c) => (
            <div key={c.key} onClick={() => setFilterCollection(c.key)} style={{
              display: "flex", alignItems: "center", padding: "7px 0", cursor: "pointer",
              opacity: filterCollection === c.key ? 1 : 0.7, transition: "opacity 0.15s",
            }}>
              {c.color && <div style={{ width: 4, height: 4, borderRadius: 2, background: c.color, marginRight: 8 }} />}
              <span style={{
                fontFamily: SANS, fontSize: 12, flex: 1,
                color: filterCollection === c.key ? C.sidebarActive : C.sidebarText,
                fontWeight: filterCollection === c.key ? 600 : 400,
              }}>{c.name}</span>
              {c.count !== undefined && <span style={{ fontFamily: MONO, fontSize: 10, color: "rgba(168,168,156,0.35)" }}>{c.count}</span>}
            </div>
          ))}
          <button style={{
            fontFamily: SANS, fontSize: 11, color: C.sidebarActive, background: "none", border: "none",
            cursor: "pointer", padding: "10px 0 0", letterSpacing: 1, opacity: 0.65,
      ...
      {/* ═══ MAIN ═══ */}
      <main style={{ flex: 1, marginLeft: 200, minHeight: "100vh", opacity: loaded ? 1 : 0, transition: "opacity 0.4s" }}>
