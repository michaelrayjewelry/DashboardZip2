...
const ORDERS = [
  {
    ...
    mfgSub: "casting", // sub-stage within manufacturing
...
// MAIN ORDERS PAGE
// ═══════════════════════════════════════
export default function ZipJewelerOrders() {
  ...
  return (
    <div style={{ fontFamily: SANS, background: C.bg, minHeight: "100vh", color: C.dark, display: "flex" }}>
      ...
      <aside style={{
        ...
        {/* Pipeline filter shortcuts */}
        <div style={{ padding: "36px 28px", flex: 1 }}>
          <div style={{ fontFamily: SANS, fontSize: 9.5, fontWeight: 600, letterSpacing: 2.5, textTransform: "uppercase", color: "rgba(168,168,156,0.45)", marginBottom: 14 }}>Pipeline</div>
          <div onClick={() => setFilterStage("all")} style={{
            padding: "7px 0", fontFamily: SANS, fontSize: 12, cursor: "pointer",
            color: filterStage === "all" ? C.sidebarActive : C.sidebarText,
            fontWeight: filterStage === "all" ? 600 : 400,
          }}>All Orders</div>
          {PIPELINE.map((s) => {
            const count = ORDERS.filter((o) => o.stage === s.key).length;
            if (count === 0) return null;
            return (
              <div key={s.key} onClick={() => setFilterStage(s.key)} style={{
                display: "flex", alignItems: "center", padding: "7px 0", cursor: "pointer",
                opacity: filterStage === s.key ? 1 : 0.7,
              }}>
                <span style={{ fontSize: 11, marginRight: 8 }}>{s.icon}</span>
                <span style={{
                  fontFamily: SANS, fontSize: 12, flex: 1,
                  color: filterStage === s.key ? C.sidebarActive : C.sidebarText,
                  fontWeight: filterStage === s.key ? 600 : 400,
                }}>{s.label}</span>
                <span style={{ fontFamily: MONO, fontSize: 10, color: "rgba(168,168,156,0.35)" }}>{count}</span>
              </div>
            );
          })}
        </div>
