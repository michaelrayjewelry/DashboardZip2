...
// ═══════════════════════════════════════
// MAIN IMAGINE COMPONENT
// ═══════════════════════════════════════
export default function ZipJewelerImagine() {
    ...
    <div style={{ fontFamily: SANS, background: C.bg, minHeight: "100vh", color: C.dark, display: "flex" }}>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        @keyframes fieldIn { from { opacity: 0; background: ${C.greenBg}; } to { opacity: 1; background: transparent; } }
      ...
      <aside style={{
        ...
        <nav style={{ display: "flex", flexDirection: "column", gap: 1 }}>
          {["Projects", "Products", "Orders", "Users"].map((item) => (
            <div key={item} style={{
              padding: "11px 28px", fontFamily: SANS, fontSize: 12.5, fontWeight: 400,
              letterSpacing: 2, textTransform: "uppercase", cursor: "pointer",
              color: C.sidebarText, background: "transparent",
              borderLeft: "3px solid transparent",
            }}>{item}</div>
          ))}
          {/* IMAGINE — highlighted */}
          <div style={{
            padding: "11px 28px", fontFamily: SANS, fontSize: 12.5, fontWeight: 600,
            letterSpacing: 2, textTransform: "uppercase", cursor: "pointer",
            color: C.sidebarActive, background: C.sidebarActiveBg,
            borderLeft: `3px solid ${C.sidebarActive}`,
            marginTop: 4,
          }}>
            ✦ Imagine
          </div>
        ...
        <div style={{ padding: "28px 28px", flex: 1, borderTop: "1px solid rgba(255,255,255,0.04)", marginTop: 20 }}>
          {started && (
            <>
              <div style={{ fontFamily: SANS, fontSize: 9.5, fontWeight: 600, letterSpacing: 2.5, textTransform: "uppercase", color: "rgba(168,168,156,0.45)", marginBottom: 14 }}>Building</div>
              <div style={{ fontFamily: SERIF, fontSize: 15, color: C.sidebarActive, letterSpacing: 2, textTransform: "uppercase", lineHeight: 1.3, marginBottom: 12 }}>
                {project.name || "New Project"}
              </div>
              <div style={{ marginBottom: 14 }}>
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                  <span style={{ fontFamily: MONO, fontSize: 9, color: "rgba(168,168,156,0.45)" }}>Readiness</span>
                  <span style={{ fontFamily: MONO, fontSize: 9, color: C.sidebarActive }}>{readiness}%</span>
                </div>
                <div style={{ height: 3, borderRadius: 2, background: "rgba(255,255,255,0.06)" }}>
                  <div style={{ height: "100%", borderRadius: 2, width: `${readiness}%`, background: C.sidebarActive, transition: "width 0.6s ease" }} />
                </div>
              </div>
              {project.type && <div style={{ fontFamily: SANS, fontSize: 11, color: C.sidebarText, marginBottom: 4 }}>{project.type}</div>}
              {project.metal && <div style={{ fontFamily: SANS, fontSize: 11, color: C.sidebarText, marginBottom: 4 }}>{project.metal}</div>}
              {project.mainGemstone && <div style={{ fontFamily: SANS, fontSize: 11, color: C.sidebarText }}>{project.mainGemstone}</div>}
            </>
          )}
        </div>
      ...
      {/* ═══ MAIN AREA ═══ */}
      <main style={{ flex: 1, marginLeft: 200, minHeight: "100vh", display: "flex", opacity: loaded ? 1 : 0, transition: "opacity 0.4s" }}>
